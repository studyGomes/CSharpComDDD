    ...
    public class UsuarioCrudCompleto : BaseTest, IClassFixture<DbTeste>
    {
        ...
        public async Task E_Possivel_Realizar_CRUD_Usuario()
        {
            using(var context = _serviceProvider.GetService<MyContext>())
            {
                ...
                {
                    Email   = Faker.Internet.Email(),   //"teste@mail.com",
                    Name    = Faker.Name.FullName()     //"teste"
                };

                var _registroCriado = await _repositorio.InsertAsync(_entity);

                Assert.NotNull(_registroCriado);
                Assert.Equal(_entity.Email      ,_registroCriado.Email  ); // "teste@mail.com"
                Assert.Equal(_entity.Name       ,_registroCriado.Name   ); //"teste"
               ...
               Assert.Equal("Alexandre Gomes"       ,_usuarioPadrao.Name   ); 
